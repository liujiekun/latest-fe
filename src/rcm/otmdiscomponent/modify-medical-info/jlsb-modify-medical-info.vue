...
<script>
import { tradeType, medicalType, dischargeStatus } from '@/rcm/otmdiscomponent/config-js/medical-regInfo-config.js'
import { leaveHospitalReason } from '@/rcm/otmdiscomponent/config-js/jlssb-regInfo-config.js'
import medicalDiagnose from '@/rcm/otmdiscomponent/medicaldiagnose.vue'
import { jlsbMedicalReg, jlsbMedicalList } from '@/rcm/store/otmdiscomponent/jlsb-api.js'
...
let rules = {
  ...
  sectionName: [{
    required: true,
    validator: (rule, value, callback) => {
      if (!value || !value.id || !value.orgSubjectCode || !value.orgSubjectName) {
...
export default {
  ...
  methods: {
    ...
    getRegisterInfo () {
      ...
      jlsbMedicalList(params).then(res => {
        let result = res.data && res.data[0]
        // 两个疾病回填
        result.inDiagnosisCode = { id: result.inDiagnosisCode, name: result.inDiagnosisName }
        result.hisDiagnosisCode = { id: result.hisDiagnosisCode, name: result.hisDiagnosisName }
        // 出院原因转换字符串
        if (result.outReason != null && result.outReason !== undefined) {
        ...
        // 医生姓名回显
        result.drName = { id: result.drCode, name: result.drName }
    ...
    medicalRegister () {
      this.$refs.form.$refs.form.validate(valid => {
        if (valid) {
          ...
          params.hisDiagnosisName = params.hisDiagnosisCode && params.hisDiagnosisCode.name
          params.hisDiagnosisCode = params.hisDiagnosisCode && params.hisDiagnosisCode.id
          // 科室
          ...
          // 医生姓名
          params.drCode = params.drName && params.drName.id
