    ...
    <div class="layout_inner">
      <div class="main-head">
        ...
        <div class="main-option">
          <el-button type="primary" @click="declare">申报</el-button>
    ...
    <el-dialog
      ...
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="申报原因">
          <el-input type="textarea" :rows="4" v-model="form.declareReason"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    declare () {
      ...
      api.medicalCostTips(params).then(rs => {
        if (!rs.data) {
    ...
    list () {
      ...
      api.searchDeclareInfo(params).then(rs => {
    ...
    confirm () {
      if (!this.form.declareReason) {
        this.$messageTips(this, 'error', '申报原因不能为空')
        return
      }
      let params = {
        month: this.$moment(this.queryObj.month).format('YYYY-MM').replace('-', ''),
        macId: this.macId,
        insuranceOrgId: this.insuranceOrgId,
        declareReason: this.form.declareReason
      }
      api.declare(params).then(rs => {
        if (rs.head.errCode === 0) {
          this.$messageTips(this, 'success', '申报成功')
          this.list()
        }
      })
    },
