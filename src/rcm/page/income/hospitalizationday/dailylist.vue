<template>
  <div class="patientList" :class="{isShowPatienList:isShow}">
    ...
    >{{feeType=='1'?'住院收费':feeType=='0'?'预交金': '住院'}}日结列表</div>
...
<script>
import api from '@/rcm/store/income/dailylist'
let schema = [
  {
    label: '日结时间',
...
export default {
  ...
  methods: {
    ...
    async list () {
      ...
      // 接口和参数都要根据feeType而发生变化了
      let result = {}
      if (this.feeType === '1') {
        result = await api.getIpCashierCloseList(params)
      } else {
        result = await api.getDepositCloseList(params)
      }
