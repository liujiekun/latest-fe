<template>
  <div>
    <ever-bread-crumb
      name="耗材出库"
      :path="`/goods/consumptiveoutputstoragerooms/${$route.params.status}`"
    ></ever-bread-crumb>
      ...
      <el-table v-loading.body="loading" :data="tableData" style="width: 100%" border>
        <el-table-column show-overflow-tooltip prop align="center" label="出库单号">
          <template slot-scope="scope">
            <span
              class="blue"
              @click="$router.push(scope.row.status == 20 ? `/goods/consumptiveoutputstorageroom/${scope.row.id}` : `/goods/consumptiveoutputstorageroomedit/${scope.row.id}?look=${scope.row.id}`)"
            >{{scope.row.code}}</span>
          </template>
        </el-table-column>
...
import { MATERIALTYPE, debounce, FULL_STATE, FULL_IN_OUT_TYPE, FULL_ORDER_NAME } from '@/util/common'
var querySchema = [
  {
...
export default {
  ...
  data () {
    ...
    return {
      ...
      FULL_ORDER_NAME,
      api,
      listParams: { 'materialType': MATERIALTYPE, status: this.$route.params.status },
      inStockType: []
    }
  },
  methods: {
    handleComplete: debounce(function () {
      Number(this.$route.params.status) === 20 ? this.$router.push('/goods/consumptiveoutputstoragerooms/21') : this.$router.push('/goods/consumptiveoutputstoragerooms/20')
