      ...
      <el-table v-loading.body="loading" :data="tableData" style="width: 100%" border>
        <el-table-column show-overflow-tooltip prop align="center" label="出库单号">
          <template slot-scope="scope">
            <span
              class="blue"
              @click="$router.push(scope.row.status == 20 ? `/goods/equipmentoutputstorageroom/${scope.row.id}` : `/goods/equipmentoutputstorageroomedit/${scope.row.id}?look=${scope.row.id}`)"
            >{{scope.row.code}}</span>
          </template>
        </el-table-column>
...
import { EQUIPMENTTYPE, debounce, FULL_STATE, FULL_IN_OUT_TYPE, FULL_ORDER_NAME } from '@/util/common'
var querySchema = [
  {
...
export default {
  ...
  data () {
    ...
    return {
      ...
      FULL_ORDER_NAME,
      api,
      listParams: { 'materialType': EQUIPMENTTYPE, status: this.$route.params.status },
      inStockType: []
    }
  },
  methods: {
    handleComplete: debounce(function () {
      this.$route.params.status === '20' ? this.$router.push('/goods/equipmentoutputstoragerooms/21') : this.$router.push('/goods/equipmentoutputstoragerooms/20')
