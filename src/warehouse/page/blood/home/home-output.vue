  ...
  <div class="home home-default boxshadownone">
    ...
    <div class="home-body taskWar">
      ...
      <div class="noCnt" v-if="emptyData">
        <everNoData tipTxt="暂无备血任务"></everNoData>
      </div>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <span class="blue" @click="$router.push(`/blood/manage/readyapply/${scope.row.id}`)">{{scope.row.patientName}}</span><span v-if="scope.row.age" style="margin-left: 10px">({{scope.row.age}}岁)</span>
            </template>
          </el-table-column>
          <!-- <el-table-column
            prop=""
...
<script>
import api from '../store/bloodpreparationapi'
...
export default {
  ...
  data () {
    let BLOOD_APPLY_STATUS = bloodconfig.BLOOD_APPLY_STATUS
    return {
      api,
      FULL_IN_OUT_TYPE,
      pagesize: 10,
      listParams: {
        status: BLOOD_APPLY_STATUS[0].id
      }
    }
  },
  mounted () {
    wsConnect('/exchange/exchange.warehouse.bloodbank/warehouse.queue.bloodPrepartion.exchange', (client, res) => {
      this.list()
    })
  },
  methods: {
