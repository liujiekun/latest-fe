...
import { DRUGTYPE, FULL_STATE, FULL_ORDER_NAME, FULL_IN_OUT_TYPE, PACKAGE_CODE, CHINESE_MEDICINE_TYPE } from '@/util/warehouseconfig'
...
export default {
  mixins: [setName, accessStorageMixins],
  data () {
  ...
  methods: {
    ...
    handleOutStorageRoom () {
      this.$refs.outStorageRoom.open('drugstoreapplylook')
    },
    getDetail (id) {
            ...
            for (let n = 0; n < res.inStockItemList[i].item.length; n++) {
              for (var j = n + 1; j < res.inStockItemList[i].item.length; j++) {
                if (res.inStockItemList[i].item[n].localMaterialId === res.inStockItemList[i].item[j].localMaterialId) {
                  ...
                  // res.inStockItemList[i].item[j].drugObject = {}
                  // res.inStockItemList[i].item[j].drugObject.drugType = result.inStockItemList[i].item[j].drugObject.drugType
                }
              }
            }
          }
      ...
      this.$refs.form.$refs.form.validate(valid => {
        if (valid) {
          ...
          this.api.createOrUpdate(params).then(res => {
            ...
            if (res) this.$router.push('/goods/drugenterstorageroomedit/' + this.objId + '?look=' + this.objId)
          })
        }
      })
    }, 1000),
    ...
    handleCalcInboundNum (data, i, index) {
      ...
      const float = /^(\+)?\d+(\.\d+)?/
      const parse = /^(\+)?\d*$/
      if (CHINESE_MEDICINE_TYPE.includes(data.localSettingObject.classifyId)) {
