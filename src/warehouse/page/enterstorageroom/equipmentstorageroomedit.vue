            ...
            >
              ...
              <el-table-column prop="name" label="商品名称">
                <template slot-scope="scope">
                  ...
                  >{{`${scope.row.equipmentObject.name || ''} ${scope.row.equipmentObject.byname ? '（' + scope.row.equipmentObject.byname + '）' : ''}${scope.row.equipmentObject.spec || ''} ${setName(typeArr, scope.row.equipmentObject.equipmentType, true)} ${setName(dosageFormArr, scope.row.equipmentObject.dosageForm, true)}`}}</span>
                  <span v-else>{{scope.row.name || scope.row.id}}</span>
                </template>
              </el-table-column>
              <el-table-column
                show-overflow-tooltip
                prop="unitGroupItemObject.name"
                width="100"
                align="center"
                label="单位"
              ></el-table-column>
              ...
              >
                <template slot-scope="scope">
                  <el-input
                    ...
                    clearable
                  ></el-input>
                  <span v-if="$route.query.look || obj.type != 10">{{scope.row.materialCode || '-'}}</span>
                </template>
              </el-table-column>
              <el-table-column
                show-overflow-tooltip
              ...
              <el-table-column show-overflow-tooltip prop width="20" label></el-table-column>
            </el-table>
            <!-- 查询添加商品 -->
            <div v-if="objId">
              <el-table
              ...
              >
                ...
                >
                  <template slot-scope="scope">
                    <el-input placeholder="设备唯一编码" v-model="scope.row.materialCode" clearable></el-input>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop label="生产批号" align="center" width="130">
                  <template slot-scope="scope">
                ...
                <el-table-column show-overflow-tooltip prop label="入库数量" align="right" width="100">
                  <template slot-scope="scope">
                    <el-input
                      placeholder="入库数量"
                      v-model="scope.row.num"
...
export default {
  ...
  methods: {
    ...
    handleOutStorageRoom () {
      this.$refs.outStorageRoom.open('equipment')
    },
    createObject () {
            ...
            for (let n = 0; n < res.inStockItemList[i].item.length; n++) {
              for (var j = n + 1; j < res.inStockItemList[i].item.length; j++) {
                if (res.inStockItemList[i].item[n].localMaterialId === res.inStockItemList[i].item[j].localMaterialId) {
                  ...
                  // res.inStockItemList[i].item[j].equipmentObject = {}
                  // res.inStockItemList[i].item[j].equipmentObject.equipmentType = result.inStockItemList[i].item[j].equipmentObject.equipmentType
                }
              }
            }
          }
        ...
        if (valid) {
          ...
          for (let i = 0; i < this.obj.inStockItemList.length; i++) {
            ...
            for (let n = 0; n < this.obj.inStockItemList[i].item.length; n++) {
              ...
              if (item.num < 0 || !item.batchNum || item.num === '' || item.num === undefined) {
                this.$messageTips(this, 'warning', '该商品入库明细入库数量小于0，或者生产批次信息未完成，请完成后再提交')
                return
              }
          ...
          this.api.createOrUpdate(params).then(res => {
            ...
            if (res) this.$router.push('/goods/equipmententerstorageroomedit/' + this.objId + '?look=' + this.objId)
          })
        }
      })
    }, 1000),
    ...
    handleAddRow (data, i) {
      if (!data['batchNum'] || !data['num']) {
        this.$messageTips(this, 'warning', '该商品该商品入库明细入库数量小于0,或者生产批次信息未完成，请完成后再添加')
        return
      }
    ...
    handleCalcInboundNum (data, i, index) {
      ...
      //       if (item1.localMaterialId === items.localMaterialId && item1.expectNum === items.expectNum) {
      //         if (items.num) expectNum += items.num
      //         if (item1.expectNum < expectNum) {
