<template>
  <div>
    <ever-bread-crumb
      name="药品入库"
      :path="obj.status == 10 ? '/goods/drugenterstoragerooms/10' : '/goods/drugenterstoragerooms/11'"
      showTitle="true"
    ></ever-bread-crumb>
...
import { DRUGTYPE, FULL_STATE, FULL_ORDER_NAME, PACKAGE_CODE, FULL_IN_OUT_TYPE, patch, SPEC_CODE } from '@/util/warehouseconfig'
...
export default {
  ...
  methods: {
    playOutStock () {
      this.$router.push('/goods/drugenterstorageroomedit/' + this.objId)
    },
    createObject () {
        ...
        if (valid) {
          ...
          for (let i = 0; i < this.obj.inStockItemList.length; i++) {
            ...
            for (let n = 0; n < this.obj.inStockItemList[i].item.length; n++) {
              delete this.obj.inStockItemList[i].item[n].drugObject
          ...
          this.api.createNew(params).then(res => {
            ...
            this.$router.push('/goods/druginboundstorage/' + res.id + '?look=' + res.id)
          })
        }
      })
    }, 1000),
