<template>
  <div>
    <ever-bread-crumb
      name="耗材入库"
      :path="`/goods/consumptiveenterstoragerooms/${$route.params.status}`"
    ></ever-bread-crumb>
      ...
      <el-table v-loading.body="loading" :data="tableData" style="width: 100%" border>
        <el-table-column show-overflow-tooltip prop align="center" label="入库单号">
          <template slot-scope="scope">
            <span
              class="blue"
              @click="$router.push(scope.row.status == 10 ? `/goods/consumptiveenterstorageroom/${scope.row.id}` : `/goods/consumptiveenterstorageroomedit/${scope.row.id}?look=${scope.row.id}`)"
            >{{scope.row.code}}</span>
          </template>
        </el-table-column>
...
import { MATERIALTYPE, FULL_STATE, FULL_ORDER_NAME, FULL_IN_OUT_TYPE } from '@/util/warehouseconfig'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      FULL_ORDER_NAME,
      FULL_IN_OUT_TYPE,
      listParams: { 'inOutstockType': 0, 'materialType': MATERIALTYPE, status: this.$route.params.status },
      inStockType: []
    }
  },
  methods: {
    addCheck () {
      this.$router.push('/goods/materialinboundstorageadd')
    },
    handleComplete: debounce(function () {
      Number(this.$route.params.status) === 10 ? this.$router.push('/goods/consumptiveenterstoragerooms/11') : this.$router.push('/goods/consumptiveenterstoragerooms/10')
