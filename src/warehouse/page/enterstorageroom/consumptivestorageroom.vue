...
import { floatTool, debounce, SPEC_CODE, patch, PACKAGE_CODE } from '@/util/common'
...
export default {
  ...
  methods: {
    playOutStock () {
      this.$router.push('/goods/consumptiveenterstorageroomedit/' + this.objId)
    },
    createObject () {
        ...
        if (valid) {
          ...
          for (let i = 0; i < this.obj.inStockItemList.length; i++) {
            ...
            for (let n = 0; n < this.obj.inStockItemList[i].item.length; n++) {
              delete this.obj.inStockItemList[i].item[n].consumptiveMaterialObject
          ...
          this.api.createNew(params).then(res => {
            ...
            this.$router.push('/goods/materialinboundstorage/' + res.id + '?look=' + res.id)
          })
        }
      })
    }, 1000),
