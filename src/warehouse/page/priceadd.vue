        ...
        <el-table-column show-overflow-tooltip prop label="类型">
          <template slot-scope="scope">
            <sys-value
              ...
              :code="scope.row.drugObject ? scope.row.drugObject.materialType : scope.row.consumptiveMaterialObject ? scope.row.consumptiveMaterialObject.materialType : scope.row.equipmentObject.materialType"
            ></sys-value>
          </template>
        </el-table-column>
        <el-table-column
...
<script>
import api from '../store/supplierprice'
...
export default {
  ...
  methods: {
    ...
    onDialogOpen () {
      if (this.queryObj.type === null) return
      this.queryObj.name = ''
      this.list()
