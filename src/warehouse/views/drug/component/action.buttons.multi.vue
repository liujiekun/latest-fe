...
export default {
  ...
  methods: {
      ...
      this.selectionTable[this.delivery].forEach(select => {
        ...
        if (mainFlag > 0) {
          ...
          if (mainFlag === 1) {
            ...
            ~index && this.metaDataList[index].taskDtoList.filter(task => (task.taskItemList[0].mainGroupId + task.createTime === compose && ((task.status === 2 && this.delivery === 0) || ([2, 3].includes(task.status) && this.delivery === 1)))).forEach(task => {
              ...
              this.printIds.drug.push({
                storage: task.expectStorageRoomId,
                id: task.id,
                type: 'drug'
              })
            })
          ...
          _sourceData.forEach(task => {
            _arr.push((localMaterialIds && (this.delivery === 0 || this.delivery === 1 && task.status === 2)) ? task.taskItemList[0].localMaterialId : task.id)
          ...
          _arr.push(select.id)
          this.printIds.drug.push({
            storage: select.expectStorageRoomId,
            id: select.id,
            type: 'drug'
          })
        }
    ...
    operatorHandler (type) {
      ...
      if (['resolve', 'complete', 'post'].includes(type) && localMaterialIds.length) {
