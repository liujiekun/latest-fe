...
<script>
  ...
  import {rulesTemplateTypeList} from './model'
  const querySchema = [
  ]
  export default {
    data: function () {
      ...
      return {
        columns: [
          ...
          },
          {
            ...
            label: '模板类型',
            formatter ({value}) {
              return listDestValue(rulesTemplateTypeList, value)
            }
          },
          {
            prop: 'name',
        ...
        querySchema,
        queryObj,
        url: urlMap.rulesTemplate.search,
      }
    },
    methods: {
      toNew () {
        this.$router.push('/systemset/rulesTemplate/detail/create')
      },
      list () {
      ...
      edit (row) {
        this.$router.push('/systemset/rulesTemplate/detail/edit/' + row.id)
      },
      del (row) {
        request(urlMap.rulesTemplate.delete, {id: row.id}).then(res => {
