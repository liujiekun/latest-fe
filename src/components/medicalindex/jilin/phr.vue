...
<script>
import mHeader from '@/components/medicalindex/jilin/phrcomponents/header'
import mBody from './phrcomponents/mainbody'
import backSide from './phrcomponents/backside'
import api from '@/workspace/store/component'
import { keyMap, form } from './phrmap'
  ...
  methods: {
    async getData () {
      ...
      try {
        ...
        if (res && res.data && typeof res.data.chineseMedicineHomePage === 'object') {
          const homeData = res.data.chineseMedicineHomePage
          for (let key in homeData) {
            this.form[key] = homeData[key] !== null ? homeData[key] : ''
          }
        }
      } catch (err) {}
      // 计算住院天数
