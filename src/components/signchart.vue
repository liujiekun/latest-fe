<template>
  <div class="chartView" :style="{'width':chartConfig.width+'px','height':chartConfig.height+'px'}" 
...
export default {
  ...
  data: function () {
    ...
    //     'nameCn': '身高',
    //     'data': [
    //       {'dateTime': '2019-04-23 00:04:12', 'value': '165'},
    //       {'dateTime': '2019-04-23 06:27:48', 'value': '120'},
    //       {'dateTime': '2019-04-25 23:56:50', 'value': '200'},
    //       {'dateTime': '2020-04-25 23:56:50', 'value': '200'}]
    //   }
    // ]
    let dataSource = null
    return {
  ...
  methods: {
    ...
    async getSignData () {
      if (!this.patientId || !this.signType) {
        console.error('请检查patientId或者signType是否传值')
      }
      let data = await phrapi.getSignChart({
    ...
    // 开始绘图,创建svg画布
      ...
      let yScale = setAxisY(this.dataSource)
    ...
    // 创建一个区域生成器
      ...
      .x(function (d, i) {
        return xScale(new Date(d.dateTime)) + padLeft
      })
      .y0(function (d, i) {
    ...
    // 创建线段生成器
      ...
      colors.forEach((item, index) => {
        linearGradients.push(setGradient(item, '#fff', 'linearGradient' + index + _this.signType))
      })
    // 数据点集合
      this.dataSource.forEach((item, index) => {
      // 添加区域
        svg.append('path')
        .attr('d', area(item.data))
        .style('fill', 'url(#' + linearGradients[index].attr('id') + ')')
      // 添加线段
        svg.append('path')
        .attr('d', line(item.data))
    ...
    // 如果把这个放到上一个循环里面会发生频闪
      this.dataSource.forEach((item, index) => {
        circles.push(drawCircle('circleWrap' + index, item.data, colors[index]))
      })
    ...
    // 鼠标移入移出数据点
      circles.forEach((item, index) => {
        ...
        circles[index].on('mouseover', function (currentNode) {
          ...
          d3.select(this).call(msover, _this.dataSource[_index].nameCn, currentNode)
        }).on('mouseout', msout)
      })
    // 浮层
      drawShadow()
    ...
    // y轴
      function setAxisY (dataSource, range) {
      // y轴比例尺
        let yData = range || getMinAndMax(dataSource, 'value')
        let max = d3.max(yData) + 20
    ...
    // 网格
      ...
      function getMinAndMax (dataSource, val) {
        ...
        dataSource.forEach(item => {
          data.push(...d3.extent(item.data, function (obj) {
    ...
    // 向数据后面多加一个日期
      ...
      function setGradient (startColor, endColor, id) {
      // 在创建区域生成器之前先创建一个颜色过渡
