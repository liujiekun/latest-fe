  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        url: [{ required: true, message: '请输入ID', trigger: 'blur' }]
      },
      isAdd: true
    }
  },
    ...
    save () {
      this.$refs.addclassify.$refs.form.validate(valid => {
        if (valid) {
          ...
          // 处理点击编辑后再点击添加的BUG
          if (this.isAdd) {
            sendObj.category = this.obj.category
            sendObj.comment = this.obj.comment
            sendObj.imageUrl = this.obj.imageUrl
            sendObj.name = this.obj.name
            sendObj.url = this.obj.url
          } else {
            sendObj = this.obj
          }
          api.createForm(sendObj, this.isAdd).then(rs => {
            if (rs.head.errCode === 0) {
