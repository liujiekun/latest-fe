<template>
  <div id="serviceImport">
    <el-dialog title="同步上级组套项目" :visible.sync="flag" class="ui_dialog_02">
...
export default {
  ...
  methods: {
    async importData () {
      ...
      if (this.$refs.selectList.servSetIds && this.$refs.selectList.servSetIds.length) {
        const res = await api.syncServiceSet({ serviceSetIds: this.$refs.selectList.servSetIds })
        if (res) {
